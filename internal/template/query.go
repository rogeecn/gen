...
const _gen_test_db_dsn = "host=localhost user=postgres password=postgres dbname=gen_test sslmode=disable"
...
func InitializeDB() {
	_gen_test_once.Do(func() {
		var err error
		_gen_test_db, err = gorm.Open(postgres.Open(_gen_test_db_dsn), &gorm.Config{})
		if err != nil {
			panic(fmt.Errorf("open postgres %q fail: %w", _gen_test_db_dsn, err))
		}
	})
}
