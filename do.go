...
func (d *DO) join(table schema.Tabler, joinType clause.JoinType, conds []field.Expr) Dao {
	...
	from.Joins = append(
		from.Joins,
		toClauseJoins(field.RelationJoin{Table: table, Type: joinType, Condition: conds})...)
...
func (d *DO) UpdateFrom(q SubQuery) Dao {
	...
	if _, ok := q.underlyingDB().Statement.Clauses["SELECT"]; ok || len(q.underlyingDB().Statement.Selects) > 0 {
		tableName.WriteString(
			"(" + q.underlyingDB().
				ToSQL(func(tx *gorm.DB) *gorm.DB { return tx.Table(q.underlyingDO().TableName()).Find(nil) }) +
				")",
		)
	} else {
...
func (d *DO) FindInBatches(dest interface{}, batchSize int, fc func(tx Dao, batch int) error) error {
	return d.db.FindInBatches(
		dest,
		batchSize,
		func(tx *gorm.DB, batch int) error { return fc(d.getInstance(tx), batch) },
	).Error
}
