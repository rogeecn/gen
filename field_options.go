...
var (
	...
	/**
	 *
	 * @param {boolean} all - If true, all basic types of fields will be replaced with their `types.Null[T]` types.
	 *                        If false, only fields that are allowed to be null will be replaced with `types.Null[T]` types.
	 *
	 * Examples:
	 *
	 * When `all` is true:
	 * - `int64` will be replaced with `types.NullInt64`
	 * - `string` will be replaced with `types.NullString`
	 *
	 * When `all` is false:
	 * - Only fields that can be null (e.g., `*string` or `*int`) will be replaced with `types.Null[T]` types.
	 *
	 * Note:
	 * Ensure that proper error handling is implemented when converting
	 * fields to their `types.Null[T]` types to avoid runtime issues.
	 */
	WithDataTypesNullType = func(all bool) model.ModifyFieldOpt {
		return func(f *model.Field) *model.Field {
			...
			case "time.Time", "string", "int", "int8", "int16",
				...
				ft = fmt.Sprintf("types.Null[%s]", ft)
			default:
				return f
			}
	...
	FieldRename = func(columnName, newName string) model.ModifyFieldOpt {
		return func(m *model.Field) *model.Field {
	...
	FieldComment = func(columnName, comment string) model.ModifyFieldOpt {
		return func(m *model.Field) *model.Field {
			if m.ColumnName == columnName {
	...
	FieldType = func(columnName, newType string) model.ModifyFieldOpt {
		return func(m *model.Field) *model.Field {
			if m.ColumnName == columnName {
				m.Type = newType
			}
	...
	FieldTypeReg = func(columnNameReg, newType string) model.ModifyFieldOpt {
	...
	FieldGenType = func(columnName, newType string) model.ModifyFieldOpt {
		return func(m *model.Field) *model.Field {
			if m.ColumnName == columnName {
	...
	FieldGenTypeReg = func(columnNameReg, newType string) model.ModifyFieldOpt {
		reg := regexp.MustCompile(columnNameReg)
		return func(m *model.Field) *model.Field {
			if reg.MatchString(m.ColumnName) {
				m.CustomGenType = newType
	...
	FieldJSONTag = func(columnName, jsonTag string) model.ModifyFieldOpt {
		return func(m *model.Field) *model.Field {
			if m.ColumnName == columnName {
