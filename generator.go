...
func (g *Generator) UseDB(db *gorm.DB) {
	if db != nil {
		if name := db.Dialector.Name(); name != "postgres" {
			panic(fmt.Errorf("unsupported database dialector %q: only postgres is supported", name))
		}
		g.db = db
	}
}
...
func (g *Generator) GenerateModelAs(tableName, modelName string, opts ...ModelOpt) *generate.QueryStructMeta {
...
func (g *Generator) genModelConfig(tableName, modelName string, modelOpts []ModelOpt) *model.Config {
	if modelOpts == nil {
...
func (g *Generator) generateModelFile() error {
	...
	for _, data := range g.models {
		...
		go func(data *generate.QueryStructMeta) {
			...
			g.info(
				fmt.Sprintf(
					"generate model file(table <%s> -> {%s.%s}): %s",
					data.TableName,
					data.StructInfo.Package,
					data.StructInfo.Type,
					modelFile,
				),
			)
		}(data)
...
func (g *Generator) pushQueryStructMeta(meta *generate.QueryStructMeta) (*genInfo, error) {
	...
	if g.Data[structName].Source != meta.Source {
		return nil, fmt.Errorf(
			"cannot generate struct with the same name from different source:%s.%s and %s.%s",
			meta.StructInfo.Package,
			meta.ModelStructName,
			g.Data[structName].StructInfo.Package,
			g.Data[structName].ModelStructName,
		)
	}
